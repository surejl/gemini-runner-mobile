...
export const Player: React.FC = () => {
  ...
  useEffect(() => {
      ...
      window.addEventListener('player-move-left', handleVirtualLeft);
      window.addEventListener('player-move-right', handleVirtualRight);
      window.addEventListener('player-jump', handleVirtualJump);
      window.addEventListener('player-ability', handleVirtualAbility);

      return () => {
          window.removeEventListener('player-move-left', handleVirtualLeft);
          window.removeEventListener('player-move-right', handleVirtualRight);
          window.removeEventListener('player-jump', handleVirtualJump);
          window.removeEventListener('player-ability', handleVirtualAbility);
      };
  }, [status, laneCount, triggerJump, activateImmortality]);
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
        ...
        if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > 30) {
            ...
            // Tap to use ability if centered (less specific than button, but kept for legacy)
            // Reduced sensitivity or reliance on virtual button is better, but keeping for now.
        }
    };
